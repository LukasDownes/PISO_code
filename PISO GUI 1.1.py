...
def update_file_buttons():
    ...
    if os.path.exists(path):
        ...
        for file in files:
            ...
            def read_and_print_file(f_path=file_path):
            ...
            if file_extension == ".txt":
                num_files += 1
                file_button = ctk.CTkButton(file_left_frame, text=file_name, command=read_and_print_file, fg_color='DarkOrchid2', width=180)
...
def open_number_pad():
    ...
    def handle_number_input():
        ...
        box_num = ctk.CTkLabel(middle_option_frame, text=f"{number}", font=("Arial", 16,"bold"))
...
# LEFT S/M/L Buttons
small_button = ctk.CTkButton(size_left_frame, text="Small", command=lambda: set_box_size("small"), fg_color=('DarkOrchid2'), width=60)
small_button.pack(side="left", pady=5)

medium_button = ctk.CTkButton(size_left_frame, text="Medium", command=lambda: set_box_size("medium"), fg_color=('DarkOrchid2'), width=60)
medium_button.pack(side="left", pady=5)

large_button = ctk.CTkButton(size_left_frame, text="Large", command=lambda: set_box_size("large"), fg_color=('DarkOrchid2'), width=60)
...
file_left_frame = ctk.CTkFrame(bottom_left_frame, fg_color="blue", width=180, height=200)
...
middle_option_frame.place(relx=0.30, rely=0.05)
...
middle_stack_frame.place(relx=0.30, rely=0.4)

# MIDDLE stack Full button
full_stack_button = ctk.CTkButton(middle_stack_frame, text="Full Stack", command=lambda: print("Full stack has been pressed"), width=190, height=50) # Assuming full stack is 12
...
right_section_frame.place(relx=0.70, rely=0.5)
